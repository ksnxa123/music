  ...
  <div class="player" v-show="playlist.length > 0">
    <transition name="max">
      <div class="max-player" v-show="fullScreen">
        <div class="background">
          ...
          <img :src="currentSong.image" alt="" height="100%" width="100%" h />
        </div>
        <div class="top">
        ...
        <div class="middle" @click="changeMiddle">
          <div class="middle-left" v-show="currentShow ==='cd'">
            <div class="cd-box">
              <div class="cd" :class="cdRotate"><img :src="currentSong.image" alt="" class="image" /></div>
            </div>
          </div>
          <scroll class="middle-right" v-show="currentShow ==='lyric'" :data="lyricList.lines" ref="lyricList">
            <div class="lyric-wrapper">
              <div class="currentLyric">
                <p class="text" ref="lyricLine" v-for="(line, index) of lyricList.lines" :class="{ current: currentLineNum === index }">{{ line.txt }}</p>
              </div>
            </div>
          </scroll>
        </div>
        <div class="bottom">
          <div class="progress-box">
            ...
            <div class="progress-bar"><progress-bar @percentChange="percentChange" :percent="percent"></progress-bar></div>
            <span class="time time-right">{{ format(duration) }}</span>
          </div>
          <div class="settings">
            <div class="icon i-left" @click="changeMode"><i class="iconfont" :class="iconMode"></i></div>
    ...
    <transition name="min">
      <div class="min-player" @click="open" v-show="!fullScreen">
        <div class="icon"><img :class="cdRotate" :src="currentSong.image" alt="" width="40" height="40" /></div>
        <div class="text">
          <h2 class="name">{{ currentSong.name }}</h2>
          <p class="desc">{{ currentSong.singer }}</p>
        </div>
        <div class="control" @click.stop="togglePlay">
          <progress-circle :radius="32" :percent="percent"><i class="iconfont icon" :class="minIcon"></i></progress-circle>
        </div>
